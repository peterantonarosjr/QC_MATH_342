...
bills_data = bills_with_payments_with_discounts %>%
  ...
  mutate(paid_in_full = if_else(sum_of_payment_amount >= total_amount, 1, 0, missing = 0)) %>%
  slice(1) %>%
  ungroup()
...
bills_data = bills_data %>%
  ...
  mutate(num_of_days_to_pay = as.integer(ymd(due_date)-ymd(invoice_date)))%>%
...
bills_data_train$y = bills_data_train$paid_in_full
bills_data_train$paid_in_full = NULL

y_train = as.factor(bills_data_train$y)
bills_data_train$y = NULL

yarfTree = YARFCART(X = bills_data_train, y = y_train, calculate_oob_error = FALSE)
```

For those of you who installed `YARF`, what are the number of nodes and depth of the tree? 

```{r}
get_tree_num_nodes_leaves_max_depths(yarfTree)
```

For those of you who installed `YARF`, print out an image of the tree.

```{r}
illustrate_trees(yarfTree, max_depth=4, open_file=TRUE)
```

Predict on the test set and report the misclassifcation error

```{r}
y_test = as.factor(bills_data_test$paid_in_full)
bills_data_test$paid_in_full = NULL

yhat = predict(yarfTree, bills_data_test)

mc_error_oos = mean(y_test != yhat)
...
With a misclassification rate of about 22% I would say this model is decent. Its not great but its better than the null model which albeit a low bar is still a bar to exceed.
...
```{r}
logistic_mod = glm(y_train ~ ., bills_data_train, family = binomial(link = "logit"))
...
rate_of_collections = .325 
cost_fn = rate_of_collections * -1000
...
```{r}
winning_thresh = which.max(performance_metrics_oos$total_cost)
