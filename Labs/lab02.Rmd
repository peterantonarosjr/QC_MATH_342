...
Xy_simple = data.frame(
 response = factor(c(0, 1)), #nominal -> Classification of "Data Points" (x1,x2)
 first_feature = c(-1, 1),    #continuous x1 -> Data Points
 second_feature = c(-1, 1)    #continuous x2 -> Data Points
)
```
...
This picture is not really satisfying because although it is separating the data points, a much "cleaner" line would be something that is perpendicular to this line.  With a perpendicular line to the one found there would be little dispute as to the separation between the two classifications.
...
Yes the SVM line is a better fit than the perceptron line.  The perceptron line leaves some "room" to interpretation about what class some points are in due to the small margin.  The SVM has a large margin between classes and so there is a clear distinction between the two.
