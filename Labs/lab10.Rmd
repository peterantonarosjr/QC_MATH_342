...
```{r}
make_missing_entries = function(X, prob_missing){
  
  for(i in 1:nrow(X)){
    for(j in 1:ncol(X)){
      
      if(runif(1) < prob_misssing){
...
```{r}
Xmiss = make_missing_entries(X,.1)
```
...
```{r}
xbars = colMeans(Xmiss, na.rm = TRUE)

Ximpnaive = Xmiss

Ximpnaive[,j] = ifelse(is.na(Xmiss[,j]), xbars[,j], Xmiss[,j])
```
...
Xmiss$chas = factor(Xmiss$chas)
XimpMF = missForest(Xmiss)$ximp
```
...
Enaive = matrix(NA, nrow(X), ncol(X))
Emf = matrix(NA, nrow(X), ncol(X))

for(i in 1:nrow(X)){
  for(j in 1:ncol(X)){
    
    if(is.na(Xmiss[i,j])){
      Enaive[i,j] = X[i,j] - Ximpnaive[i,j]
      Emf[i,j] = X[i,j] - XimpMF[i,j]
    }
  }
}
```
