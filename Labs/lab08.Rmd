...
```{r}
storms[,storm_number:=uniqueN(name),by=name]
storms[,.(name,storm_number)]
```
...
Xy
=======
storms[, .SD[1:3], by = name]
Xy = first_three_readings = storms[, .(last_status = .SD[3,status], max_category = max(category), max_wind_thus_far = max(wind), min_pressure = min(pressure), max_pressure = max(pressure), y=max_wind_speed), by=.(name,year)]
...
bills_with_payments_with_discounts = merge(bills_with_payments,discounts, all.x=TRUE, by.x="id", by.y="id")
