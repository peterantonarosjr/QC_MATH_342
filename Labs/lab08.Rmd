...
```{r}
storms[,average_diameter:= pmean(c("tropicalstorm_force_diameter", ""))]
```
...
```{r}
storms[,.(max_category = max(category, na.rm = TRUE), max_wind = max(wind, na.rm = TRUE), max_pressure = max(pressure, na.rm = TRUE), max_ts_diam = max(tropicalstorm_diameter, na.rm = TRUE), max_hu_diam = max(hurricane_force_diameter, na.rm = TRUE)),by=name]
```
