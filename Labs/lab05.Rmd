...
#max(caratSubset$carat) # Check the max carat size to ensure we are selecting right thing
...
y = caratSubset$price
priceClarity = lm(y~x)
standardError = sd(priceClarity$residuals)
standardError
```
...
which means fitting a line to this will most likely have more error than fitting to a continuous variable such as carat. (I believe its continous, or maybe very granular discrete I can't tell nor do I know that much about diamonds)
...
```{r}
mod1 = lm(diamonds$price ~ diamonds$carat)
mod2 = lm(diamonds$price ~ ln_carat)
...
```{r}
modf = lm(ln_price ~ (ploy(carat,3) + poly(x,3) + plot(y,3) + polt(z,3) + polt(depth,3) + poly(table,3)) * (clarity + cut  + color), diamonds)
summary(modf)$sigma
```
...
set.seed(1984)
sampleNum = 2000
D = diamonds[sample(1:nrow(diamonds),sampleNum)]
Dtrain = D[1: ((1-1/k)*sampleNum),]
Dtest = D-Dtrain #POSSIBLY WRONG
```
...
outSampleRMSE[['A']] = sd(Dtest$ln_price - predict(moda,Dtest))
